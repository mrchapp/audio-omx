                           ...
                           OMX_PTR pEventData)
{
  ...
  case OMX_EventBufferFlag:
    if((nData2 == (int)OMX_BUFFERFLAG_EOS) && (nData1 == OMX_DirInput)){   //if((nData2 == (int)OMX_BUFFERFLAG_EOS) && (nData1 == OMX_DirInput)){
...
static int parameter_check(int argc,char **argv, appPrivateSt *appPrvt)
{
    ...
    if(argc < 2){
        ...
        if(strcasecmp((inputFile+fn_index -4),".pcm")!=0){
            //appPrvt->eEncoding=OMX_AUDIO_CodingMP3;
            perror("Input file format should be pcm\n");
        	exit(1);
        }
		...
		//must validate output file existance here
		printf ("\noutputFile %s\n", outputFile);
		fn_index = strlen(outputFile);
		if(fn_index < 4){
			printf("Invalid filename \n");
		    exit(1);
		}
		/*if(strcasecmp((outputFile+fn_index -4),".mp3")!=0){
		    //appPrvt->eEncoding=OMX_AUDIO_CodingMP3;
		    perror("Input file format should be pcm\n");
		  	exit(1);
		}*/
		else if(strcasecmp((outputFile+fn_index -4),".aac")==0){
...
static int alloc_app_resources(appPrivateSt* appPrvt)
{
    ...
    appPrvt->out_port->format.audio.eEncoding = appPrvt->eEncoding;
...
{
    OMX_ERRORTYPE error = OMX_ErrorNone;
    OMX_STRING audio_encode_string = "OMX.TI.AUDIO.ENCODE";
    ...
    /*case OMX_AUDIO_CodingMP3:
        ...
        break;*/
    case OMX_AUDIO_CodingAAC:
        strcpy((char*)appPrvt->pCompRoleStruct->cRole,"audio_encode.dsp.aac");
        break;
    default:
        strcpy((char*)appPrvt->pCompRoleStruct->cRole,"audio_encode.dsp.aac");
        break;
