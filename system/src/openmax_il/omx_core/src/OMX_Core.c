...
char *tComponentName[MAXCOMP][2] = {
    ...
    /*TI base sample component*/
    {"OMX.TI.Sample", NULL},
    {"OMX.TI.AUDIO.DECODE", "audio_decode.dsp.aac"},
    {"OMX.TI.AUDIO.DECODE", "audio_decode.dsp.mp3"},
    {"OMX.TI.AUDIO.ENCODE", "audio_encode.dsp.aac"},
...
OMX_API OMX_ERRORTYPE OMX_PrintComponentTable()
{
    ...
    printf("------------Component Table:: %d Components found---------------\n", tableCount);

    for (i = 0; i < tableCount; i++)
    {
        printf("%i:: %s\n", i, componentTable[i].name);
       for (j = 0; j<componentTable[i].nRoles; j++) 
       {
