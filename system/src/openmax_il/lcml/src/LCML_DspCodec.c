...
NodeAttrIn.uTimeout = 1000; /* WORKAROUND */       
        ...
        phandle->nInputBufContMem = 0;
        phandle->nOutputBufContMem = 0;
                                  ...
                                  OMX_S32 auxInfoLen ,OMX_U8 * usrArg )
{
    ...
    if ((buffer != NULL) && (bufferLen != 0))
    {
        ...
        if (bufType == EMMCodecInputBuffer || !(streamId % 2)) {
            eError = DmmMap(phandle->dspCodec->hProc, bufferLen, bufferSizeUsed,buffer, (pDmmBuf), phandle->nInputBufContMem);
        }
        else if (bufType == EMMCodecOuputBuffer || streamId % 2) {
            eError = DmmMap(phandle->dspCodec->hProc, bufferLen, bufferLen,buffer, (pDmmBuf), phandle->nOutputBufContMem);
        }
    ...
    if (auxInfoLen != 0 && auxInfo != NULL )
    {
        ...
        eError = DmmMap(phandle->dspCodec->hProc, phandle->commStruct->iParamSize, phandle->commStruct->iParamSize, (void*)phandle->commStruct->iParamPtr, (pDmmBuf), OMX_FALSE);
        if (eError != OMX_ErrorNone)
        {
            goto EXIT;
        }
    ...
    eError = DmmMap(phandle->dspCodec->hProc, sizeof(TArmDspCommunicationStruct),sizeof(TArmDspCommunicationStruct), (void *)phandle->commStruct, (pDmmBuf), OMX_FALSE);
    if (eError != OMX_ErrorNone)
    {
        goto EXIT;
    }
                                  ...
                                  void * args[10])
{
    ...
    switch (iCodecCmd)
    {
        ...
        case EMMCodecControlAlgCtrl:
        {
            ...
            eError = DmmMap(phandle->dspCodec->hProc,(int)args[2],(int)args[2], args[1],(phandle->pAlgcntlDmmBuf[i]), OMX_FALSE);
            if (eError != OMX_ErrorNone)
            {
                goto EXIT;
            }
        ...
        case EMMCodecControlStrmCtrl:
        {
            ...
            else 
            {
                ...
                eError = DmmMap(phandle->dspCodec->hProc, (int)args[2],(int)args[2], args[1],(phandle->pStrmcntlDmmBuf[i]), OMX_FALSE);
                if (eError != OMX_ErrorNone)
                {
                    goto EXIT;
                }
        ...
        case EMMCodecControlContiguousMem:
        {
            if ((int)args[0] == 0){
                    phandle->nInputBufContMem = (int)args[1];
            }
            else if ((int)args[0] == 1){
                    phandle->nOutputBufContMem = (int)args[1];
            }
        }
    }
                     ...
                     OMX_U32 nContiguousMem)
{
    ...
    /* Map */
    if (nContiguousMem == 1) {
        printf("DSPProcessor_Map is using contiguous memory\n");
        ulMapAttr = DSP_MAP_PHYSBUFF_CONTIGUOUS;
    }
    else if(nContiguousMem == 2) {
        printf("DSPProcessor_Map is using mostly contiguous memory\n");
        ulMapAttr = DSP_MAP_PHYSBUFF_64K_CONTIGUOUS;
    }
    else { 
        ulMapAttr = 0;
    }
    ...
    status = DSPProcessor_Map(ProcHandle,
                          ...
                          ulMapAttr); /* final param is used to set contigous memory*/
    if(DSP_FAILED(status))
    {
